\setlength{\parindent}{0cm}%EL ANCHO DE LA SANGRIA DE AQUÍ EN ADELANTE%
\hoffset-0.460cm
\voffset-1.46cm
\begin{window}[0,15,{\includegraphics[scale=0.35]{logoverdeUNAL.png}},]
\scshape
\thispagestyle{plain}

\LARGE\hspace{1cm}\textsc{Universidad Nacional de Colombia}\\
\textcolor{white}{\tiny} \hspace{2cm}\textsc{\small Facultad de Ciencias - Departamento de Matemáticas}\\
\textcolor{white}{\tiny} \hspace{2.0cm}\textsc{\small \textbf{Teoría de cuerpos 2025-I}\\}
\Large\hspace{0.00002cm}\\\\
\end{window}
\normalfont
\normalsize 
\begin{center}
\hspace{3.6cm}\dotfill
\Large\hspace{0.001cm} \\
\vspace{0.25cm}
\begin{minipage}{6cm}
    \small{Cristian Camilo Pérez Puentes}
\end{minipage}
\begin{minipage}{5cm}
    \small{\texttt{crozol@unal.edu.co\\crperezp@unal.edu.co\\jcadenap@unal.edu.co}}
\end{minipage}\\
\vspace{0.25cm}
\hrulefill\\

\end{center}